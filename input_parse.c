...
/** реализация массива зарезервированных идентификаторов, если такое слово будет встречено в строке,
  * оно будет интерпретироваться как оператор,  функция или математическая константа **/
...
/// возвращает код встреченного символа, если он представляет собой оператор иначе возвращает код none
...
///  возвращает код встреченного слова, если строка не является зарезервированным словом, то возвращается none
 ...
 *  если совпадений нет, то добавляет встреченное имя в конец пула и возвращает индекс **/
...
/** проверка на то, что встреченный символ является цифрой, десятичной точкой или 'j',
  * только эти символы могут входить в и начинать числовую константу **/
int isDigitOrJ(char c) {
    return '0' <= c && c <= '9' || c == '.' || c == 'j';
}
/** в идентификаторах разрушены буквы любого регистра и цифры ( не лидирующие )
  * метод проверяет переданный символ и возвращает результат проверки **/
...
/** главный метод парсинга стооки, по началу каждого "слова" определяет тип токена,
